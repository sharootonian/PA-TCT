function obj=SimM2(B_data1,B_data2,data1,data2)
% simulation 
    for(k= 1:100)
     sb1(k).sim=sim_beta_m2(B_data1);
     sb2(k).sim=sim_beta_m2(B_data2);

     %simulate x3 from model0 
        for(i= 1:length(data1.subj))
            md1(i).subj(k).sim=m2s(sb1(k).sim(i,:),data1.subj(i).vec3,data1.subj(i).vec1,data1.subj(i).vec2,data1.subj(i).vec1p,data1.subj(i).vec2p,data1.subj(i).vec3p,data1.subj(i).Trial,data1.subj(i).size);
        end
        for(i= 1:length(data2.subj))
            md2(i).subj(k).sim=m2s(sb2(k).sim(i,:),data2.subj(i).vec3,data2.subj(i).vec1,data2.subj(i).vec2,data2.subj(i).vec1p,data2.subj(i).vec2p,data2.subj(i).vec3p,data2.subj(i).Trial,data2.subj(i).size);
        end
    end
