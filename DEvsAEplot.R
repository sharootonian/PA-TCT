DEvsAEplot<-function(data,exp,model){
    if(exp==1)
    {
  ...
  if(model=="data"){
    ...
    p <- ggplot(data, aes(M1DE,M1AE,color=color))+ geom_point(size=4,show.legend = FALSE) + theme_classic()+ geom_vline(xintercept=1, linetype="dashed",color = "red", size=1) +geom_hline(yintercept=0, linetype="dashed",color = "red", size=1)
    p <-p + labs(x = "Distance error (ratio)")
    p <-p + labs(y = "Angle error (deg)")
    p <- p + labs(title = title)+
      theme_light()+
    ...
    p <- ggplot(data, aes(M2DE,M2AE,color=color))+ geom_point(size=4,show.legend = FALSE) + theme_classic()+ geom_vline(xintercept=1, linetype="dashed",color = "red", size=1) +geom_hline(yintercept=0, linetype="dashed",color = "red", size=1)
    p <-p + labs(x = "Distance error (ratio)")
    p <-p + labs(y = "Angle error (deg)")
    p <- p + labs(title = title)+
      theme_light()+
    ...
    p <- ggplot(data, aes(EEMDE,EEMAE,color=color))+ geom_point(size=4,show.legend = FALSE) + theme_classic()+ geom_vline(xintercept=1, linetype="dashed",color = "red", size=1) +geom_hline(yintercept=0, linetype="dashed",color = "red", size=1)
    p <-p + labs(x = "Distance error (ratio)")
    p <-p + labs(y = "Angle error (deg)")
    p <- p + labs(title = title)+
      theme_light()+
